 ...
 * @version $Id: finalization.php 116 2010-04-18 10:08:21Z nikosdion $
 */
...
class AECoreDomainFinalization extends AEAbstractPart
{
	...
	private function run_post_processing()
	{
		...
		if($result === false)
		{
			AEUtilLogger::WriteLog(_AE_LOG_DEBUG, 'Failed to process file '.$filename);
		}
		elseif($result === true)
		{
			AEUtilLogger::WriteLog(_AE_LOG_DEBUG, 'Successfully processed file '.$filename);
		}
		else
		{
			AEUtilLogger::WriteLog(_AE_LOG_DEBUG, 'More processing required for file '.$filename);
		}
		...
		if($result === true) $this->backup_parts_index++;
		// We are past the end of the array, so we're finished
