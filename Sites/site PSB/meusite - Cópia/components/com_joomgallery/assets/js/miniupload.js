...
function joom_singleuploadcheck()
{
  ...
  if(arenofiles) {
    alert(JText._('JGS_IMGMAN_CHOOSE_IMAGE'));
  ...
  if(extensionsnotok) {
    alert(JText._('JGS_UPLOAD_ALERT_WRONG_EXTENSION'));
    document.getElementsByName(screenshotfieldname[0])[0].focus();
    return false;
  ...
  if(filenamesnotok) {
    alert(JText._('JGS_UPLOAD_ALERT_WRONG_FILENAME'));
    document.getElementsByName(screenshotfieldname[0])[0].focus();
    return false;
...
if(!$this->_config->get('jg_useorigfilename') && $this->_config->get('jg_filenamenumber'))
{
  ...
  if (form.filecounter.value != '') {
    ...
    if(searchwrongchars1.test(form.filecounter.value)) {
      filecounterok = false;
      alert(JText._('JGS_UPLOAD_ALERT_WRONG_VALUE'));
