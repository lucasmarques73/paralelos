...
function read_bookmarks() {
	...
	if( file_exists( $bookmarkfile )) {
		...
		if( !is_writable( dirname( $bookmarkfile ) )) {
			...
			file_put_contents( $bookmarkfile, ";<?php if( !defined('_JEXEC') or die('Restricted access'); ?>\n{$GLOBALS['messages']['homelink']}=\n" );
			return array( $GLOBALS['messages']['homelink'] => '' );
		}
	}
}
...
function modify_bookmark( $task, $dir ) {
	...
	switch ( $task ) {
		case 'add':
			...
			if( in_array( $dir, $bookmarks )) {
				echo nx_alertBox( $GLOBALS['messages']['already_bookmarked'] ); exit;
			}
		...
		case 'remove':
			...
			if( !in_array( $dir, $bookmarks )) {
				echo nx_alertBox( $GLOBALS['messages']['not_a_bookmark'] ); exit;
			}
	...
	if( !is_writable( $bookmarkfile )) {
		echo nx_alertBox( sprintf( $GLOBALS['messages']['bookmarkfile_not_writable'], $task, $bookmarkfile ) ); exit;
	}
...
function list_bookmarks( $dir ) {
	...
	if( !isset( $bookmarks[$dir] ) && $dir != '' && $dir != '/' ) {
		$addlink = '<a href="'.make_link('modify_bookmark', $dir ).'&task=add" onclick="var alias = prompt(\''.$GLOBALS['messages']['enter_alias_name'].':\', \''.$dir.'\');if( alias==\'\' || alias == null )return false; adder = new Ajax(\''.make_link('modify_bookmark', $dir ).'&task=add&alias=\' + alias, { method: \'get\', postBody: \'action=modify_bookmark&task=add&dir='.$dir.'&alias=\' + alias + \'&option=com_ninjaxplorer\', evalScripts:true, update: \'quick_jumpto\' } );adder.request(); return false;" title="'.$GLOBALS['messages']['lbl_add_bookmark'].'" >'.$img_add.'</a>';
	}
	elseif( $dir != '' && $dir != '/' ) {
		$removelink = '<a href="'.make_link('modify_bookmark', $dir ).'&task=remove" onclick="remover = new Ajax(\''.make_link('modify_bookmark', $dir ).'&task=remove\', { method: \'get\', update: \'quick_jumpto\', postBody: \'action=modify_bookmark&task=remove&dir='.$dir.'&option=com_ninjaxplorer\', evalScripts:true } );remover.request(); return false;"  title="'.$GLOBALS['messages']['lbl_remove_bookmark'].'">'.$img_remove.'</a>';
	}
