 ...
 * @version $Id: jpa.php 124 2010-04-27 16:30:56Z nikosdion $
 */
...
class AEArchiverJpa extends AEAbstractArchiver
{
	...
	{
		AEUtilLogger::WriteLog(_AE_LOG_DEBUG, "AkeebaPackerJPA :: new instance - archive $targetArchivePath");
		...
		if($fragmentsize >= 65536)
		{
			...
			AEUtilLogger::WriteLog(_AE_LOG_INFO, "AkeebaPackerJPA :: Spanned JPA creation enabled");
		...
		AEUtilLogger::WriteLog(_AE_LOG_DEBUG, "AkeebaPackerJPA :: Killing old archive");
				...
				else
				{
					...
					else
					{
						...
						while($bytes_left > 0)
						{
							...
							// Split between parts - Write first part
							$this->_fwrite( $fp, $sourceNameOrData, min(strlen($sourceNameOrData), $free_space) );
							if($this->getError()) {
								@fclose($fp);
								return;
							}
				...
				else
				{
					...
					if(!$this->_useSplitZIP)
					{
						while( !feof($zdatafp) && ($timer->getTimeLeft() > 0) && ($unc_len > 0) ) {
							...
							$unc_len -=  min(strlen($zdata), AKEEBA_CHUNK);
							if($this->getError()) {
								@fclose($zdatafp);
								@fclose($fp);
								return;
						...
						else
						{
							...
							while( !feof($zdatafp) && ($timer->getTimeLeft() > 0) )
							{
								...
								for( $i = 1; $i <= $loop_times; $i++ )
								{
									...
									$unc_len -= min(strlen($zdata), $chunk_size_primary);
									if($this->getError()) {
										@fclose($zdatafp);
										@fclose($fp);
										return;
								...
								if($chunk_size_secondary > 0)
								{
									...
									$unc_len -= min(strlen($zdata), $chunk_size_secondary);
									if($this->getError()) {
										@fclose($zdatafp);
										@fclose($fp);
										return;
