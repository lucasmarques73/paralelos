 ...
 * @version $Id: native.php 124 2010-04-27 16:30:56Z nikosdion $
 */
	...
	protected function _run()
	{
		...
		if( ($this->nextRange < $this->maxRange) )
		{
			...
			else
			{
				...
				$db->setQuery( $sql, $this->nextRange, 1000 );
...
// =============================================================================
	...
	private function get_tables_mapping()
	{
		...
		{
			AEUtilLogger::WriteLog(_AE_LOG_DEBUG, __CLASS__." :: Ready to filter");
			$table_abstract = $this->getAbstract($table_name);
			if( substr($table_abstract,0,4) != 'bak_' ) // Skip backup tables
			{
				// Apply exclusion filters
				if( !$filters->isFiltered($table_abstract, $root, 'dbobject', 'all') ) $this->table_name_map[$table_name] = $table_abstract;
			}
			AEUtilLogger::WriteLog(_AE_LOG_DEBUG, __CLASS__." :: Filtering complete");
		}
	 ...
	 */
	private function push_table($table_name, $stack = array())
	{
		// Load information
			...
			if(array_key_exists($table_name, $this->dependencies))
			{
				foreach($this->dependencies[$table_name] as $depended_table)
				{
					...
					if( ($remove_position !== false) && ($remove_position < $my_position) )
					{
						...
						// I have to recurse...
						$this->push_table($depended_table);
					} // if remove_position
				} // foreach
			} // if in dependencies
