 ...
 * @version $Id: default.php 113 2010-04-13 21:45:32Z nikosdion $
 * @since 3.0
 */
...
<form name="adminForm" method="post" action="<?php echo JURI::base()?>index.php" >
...
<script type="text/javascript">
	...
	akeeba.jQuery(document).ready(function($){
		...
		akeeba_translations['UI-BROWSE'] = '<?php echo addcslashes(JText::_('CONFIG_UI_BROWSE'),"'\\") ?>';
		akeeba_translations['UI-CONFIG'] = '<?php echo addcslashes(JText::_('CONFIG_UI_CONFIG'),"'\\") ?>';
		...
		directftp_test_connection = function()
		{
			...
			// Construct the query
			akeeba_ajax_url = '<?php echo addcslashes(JURI::base().'index.php?option=com_akeeba&view=config&format=raw&ajax=testftp',"'\\") ?>';
			doAjax(data, function(res){
				...
				var dialog_element = $("#dialog");
				dialog_element.html(''); // Clear the dialog's contents
				dialog_element.dialog('option', 'title', '<?php echo addcslashes(JText::_('CONFIG_DIRECTFTP_TEST_DIALOG_TITLE'),"'\\") ?>');
				dialog_element.removeClass('ui-state-error');
				if( res === true )
				{
					$(document.createElement('p')).html('<?php echo addcslashes(JText::_('CONFIG_DIRECTFTP_TEST_OK'),"'\\") ?>').appendTo(dialog_element);
				}
				else
				{
					$(document.createElement('p')).html('<?php echo addcslashes(JText::_('CONFIG_DIRECTFTP_TEST_FAIL'),"'\\") ?>').appendTo(dialog_element);
...
<?php if(defined('AKEEBA_PRO')): ?>
		...
		postprocftp_test_connection = function()
		{
			...
			doAjax(data, function(res){
				...
				var dialog_element = $("#dialog");
				dialog_element.html(''); // Clear the dialog's contents
				dialog_element.dialog('option', 'title', '<?php echo addcslashes(JText::_('CONFIG_POSTPROCFTP_TEST_DIALOG_TITLE'),"'\\") ?>');
				dialog_element.removeClass('ui-state-error');
				if( res === true )
				{
					$(document.createElement('p')).html('<?php echo addcslashes(JText::_('CONFIG_POSTPROCFTP_TEST_OK'),"'\\") ?>').appendTo(dialog_element);
				}
				else
				{
					$(document.createElement('p')).html('<?php echo addcslashes(JText::_('CONFIG_POSTPROCFTP_TEST_FAIL'),"'\\") ?>').appendTo(dialog_element);
					$(document.createElement('p')).html( res ).appendTo( dialog_element );
				}
				dialog_element.dialog('open');
			});
...
<?php endif; ?>
		...
		akeeba_browser_hook = function( folder, element )
		{
			...
			dialog_element.html(''); // Clear the dialog's contents
			dialog_element.removeClass('ui-state-error');
			dialog_element.dialog('option', 'title', '<?php echo addcslashes(JText::_('CONFIG_UI_BROWSER_TITLE'),"'\\") ?>');

			// URL to load the browser
			var browserSrc = '<?php echo addcslashes(JURI::base().'index.php?option=com_akeeba&view=browser&format=raw&processfolder=1&folder=', "'\\") ?>';
